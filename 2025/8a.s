...
.calculateDistanceInnerLoop:
  ...
  ; for each of the pairs to operate on, merge the circuits (*left.circuit += *right.circuit; *right.circuit = 0, forall nodes where circuit = right, node.circuit = left)
  mov rcx, 0
.joinLoop:

  ; rdi = pointer to distance
  mov rdi, rcx
  shl rdi, 5
  add rdi, distances

  ; get pointers to boxes
  mov r8, [rdi + distanceBox1Offset]
  mov r9, [rdi + distanceBox2Offset]

  ; try linking these - if they're already part of the same circuit, do nothing
  mov rdi, [r8 + boxCircuitOffset]
  mov rsi, [r9 + boxCircuitOffset]
  cmp rdi, rsi
  je .continueJoinLoop

  ; add the count of boxes on circuit 2 to circuit 1, zero out circuit 2
  mov rax, [rsi]
  add [rdi], rax
  mov QWORD [rsi], 0
...
.continueJoinLoop:
  ...
  cmp rcx, numConnections
  jb .joinLoop

.breakpoint:
  mov rdi, circuits
  lea rsi, [circuits + length * 8]
  call qsort

  ; calculate largest
  mov rax, [circuits + length * 8 - 1 * 8]
  mov rdi, [circuits + length * 8 - 2 * 8]
  mul rdi
  mov rdi, [circuits + length * 8 - 3 * 8]
  mul rdi

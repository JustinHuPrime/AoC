...
_start:
  ...
  ;;   unsigned qword count;
  ;; }
  %define sizeofNode 4 * 8
  %define prevNode 0
  %define nextNode 8
  %define dataNode 16
  %define countNode 24
  ...
  mov QWORD [listHead + countNode], 0
...
.notZero:
  ...
  mov rdi, [curr + countNode]
  mov [rax + countNode], rdi
...
.coalesceLoop:
  ...
  mov rax, [curr + dataNode]
.coalesceElementLoop:
  mov search, [search + nextNode]
  cmp search, listTail
  je .endCoalesceElementLoop

  ; check - is this element equal to curr
  cmp rax, [search + dataNode]
  jne .coalesceElementLoop ; if not, skip

  mov rdx, [search + countNode] ; add on to the count
  add [curr + countNode], rdx

  ; delete this node
  mov rdi, [search + nextNode]
  mov rsi, [search + prevNode]
  mov [rdi + prevNode], rsi
  mov [rsi + nextNode], rdi

  jmp .coalesceElementLoop
.endCoalesceElementLoop:

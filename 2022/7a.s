...
;; returns sum of sizes of all directories with size <= 100000
sumsmall:
  bt QWORD [rdi + 32], 0
  jc .fileReturn
...
.loop:
  mov rdi, [rsp + 0] ; rdi = this
  cmp rsi, [rdi + 16] ; rsi < this->numentries?
  jnl .endLoop

  push rsi
  mov rdi, [rdi + 24] ; rdi = this->entries
  mov rdi, [rdi + rsi * 8] ; rdi = this->entries[rsi]
  call sumsmall
  pop rsi
...
.endLoop:
  ...
  mov rax, [rsp + 8]
  mov rdx, rax
  add rdx, [rdi + 8] ; get this->size
  cmp QWORD [rdi + 8], 100000 ; if this->size <= 100000, add to return value
  cmovle rax, rdx
